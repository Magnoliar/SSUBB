...
1. **单卡串行处理**：目前 Worker 端为了保证显存不溢出，所有任务都是排队串行处理的，暂不支持多任务绝对并发（但音频提取与转写在不同节点时可流水线重叠）。
...
4. **纯内网分发**：当前版本未内置强鉴权机制（Token），请**不要将其直接暴露在公网**，务必搭配局域网、Tailscale 或带认证的 Nginx 反向代理使用。
